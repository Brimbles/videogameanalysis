...
import numpy as np  # mostly for numpy arrays. 
import time
...
#soup = BeautifulSoup(content, fromEncoding='latin-1') 

for page in range(1, pages):
    surl = urlhead + str(page) + urltail
    r = urllib.request.urlopen(surl).read()
    soup = BeautifulSoup(r,  features="html.parser")
    #soup = soup.encode("utf-8") #added to try to handle issues with the super robot a character thing 
    #soup = soup(decode) #decode('utf-8','ignore')
    print(f"Page: {page}")  # this is to show progress in the terminal
    time.sleep(5) #added this in as getting a HHTP error 429 (too many requests)
    # print(soup)
...
#testing content of soup
    # df = pd.DataFrame(game_tags)
    # df.to_csv("game_tags.csv", sep=",", encoding='utf-8', index=False)

#testing content of soup
    # df= pd.DataFrame(soup)
    # df.to_csv("vgsalestest.csv", sep=",", encoding='utf-8', index=False)
#######################################
    ...
    for tag in game_tags:
        ...
        #print(f"{rec_count + 1} Fetch data for game {gname[-1]}") #Original#Original#Original#Original#Original#Original
        # gnameforprinting = str(rec_count + 1) + " Fetch data for game " + gname[-1].replace("\u03b1", "-")
        # print(gnameforprinting)
        print(str(rec_count + 1) + " Fetch data for game " + gname[-1].replace("\u03b1", "-").replace("\u200b", "-").replace("\u266a", "-"))
        

        time.sleep(0.5) #added this in as getting a HHTP error 429 (too many requests) #############################
